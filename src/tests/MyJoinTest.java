...
class MyJoinsDriver implements GlobalConst {
	...
	*/
	...
	/**
	* Run the query of task 2a. IE Self Join + Correctness with Nested Loop Join with 1 predicate
	* 
	...
	*/
		...
		try {
			...
			while ((t = ieSF.get_next()) != null) {
				...
				if (verbose) {
					...
					System.out.print("\n" + "IE Self Join : " + i + "       "); // Print the rank of the tuple
					t.print(jtype); // Print the tuple to the console
			...
			if (correctness_NLJ) {
				...
				while ((t = nlj.get_next()) != null) {
					...
					if (verbose) {
						...
						System.out.print("\n" + "NLJ : " + j + "       "); // Print the rank of the tuple
						t.print(jtype); // Print the tuple to the console
	...
	/**
	* Run the query of task 2b. IE Self Join + Correctness with Nested Loop Join with 2 predicates
	*
	* @param  correctness_NLJ : boolean set to <code> true </code> if you want to compare the resulting tuples with NLJ
	* @param  verbose : boolean set to <code> true </code> if you want to print the resulting tuples for both IE and NLJ (if chosen to correct the results)
	*/
	...
	/**
	* Run the query of task 2b. IE Join + Correctness with Nested Loop Join with 2 predicates
	* 
	* @param  correctness_NLJ : boolean set to <code> true </code> if you want to compare the resulting tuples with NLJ
	* @param  verbose : boolean set to <code> true </code> if you want to print the resulting tuples for both IE and NLJ (if chosen to correct the results)
	*/
		...
		try {
			...
			while ((t = ieJoin.get_next()) != null) {
				...
				if (verbose) {
					...
					System.out.print("\n" + "IE Join : " + i + "       "); // Print the rank of the tuple
					t.print(jtype); // Print the tuple to the console
...
public class MyJoinTest {
	...
	public static void main(String[] args) {
		...
		int max_num_tuples_Q = 500; // Don't get it too much... Heap storage can support until 30 000 but more than 2500 start to make joins very long. 500 is nice for fast debug for example
	    List<String> queries = Arrays.asList("2b"); // List here the queries you want to test : "1a","1b","2a","2b" or "2c"
	    boolean correctness = true; // If you want to compare NLJ with the IE Self Join of task 2a and 2b and IE Join from task 2c
	    boolean verbose = true; // If you want to print the resulting tuples while the queries are running.
